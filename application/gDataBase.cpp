...
void gDataBase::database_init()
{
    ...
    QFileInfo checkfile(QString("%1/%2").arg(tmpString).arg("msgDatabase.sqlite"));
    m_profile_database = QSqlDatabase::addDatabase("QSQLITE");
    m_profile_database.setDatabaseName(tmpString + "/msgDatabase.sqlite");
...
//    open_database();
    ...
    if(m_query.exec("CREATE TABLE IF NOT EXISTS " +  m_profile_table_name +
                 " (id INTEGER PRIMARY KEY, param_name TEXT, param_type NUMERIC)"))
        return true;
    else return false;
}
...
void gDataBase::save_one_parameter(int param_id, QString param_name, int param_type)
{
    QSqlQuery m_query(m_profile_database);
    ...
    m_query.addBindValue(param_type);
...
int gDataBase::get_type_parameter(int id)
{
    int result = 0;
    QSqlQuery m_query(m_profile_database);
    m_query.prepare("SELECT param_type FROM " + m_profile_table_name + " WHERE id=?;");
//    m_query.addBindValue(param_class);
    m_query.addBindValue(id);

    if(m_query.exec()){
        while(m_query.next()){
            result =  m_query.value(m_query.record().indexOf("param_type")).toInt();
        }
    }
    else result =  0;
    return result;
